...
// 위를 줄이면 아래와 같다.
// var or val 을 생성자의 인자에 선언 시 전달받은 값이 자동으로 같은 이름의 필드가
// 만들어 지면서 값이 필드에 대입된다.
// var 은 수정이 가능한 필드 , val 은 수정이 불가능한 필드가 만들어 진다.
...
fun main(){
    ...
    // Type 추론이 가능하기에 members2 의 Type 생략 가능
    val members2 = mutableListOf<Member>()

    // List 의 add 메소드를 이용해서 Member 객체의 참조값을 저장하기
    members.add(m1)
    members.add(Member(2 , "해골" , "행신동"))

    // List 의 forEach() 함수를 호출하면서 함수를 전달하면 전달한 함수의 매개변수에
