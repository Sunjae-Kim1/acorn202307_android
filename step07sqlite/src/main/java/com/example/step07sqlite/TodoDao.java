...
public class TodoDao {
    ...
    public void insert(Todo todo){
        ...
        // ? 에 순서대로 Binding 인자를 Object[] 에 준비한다.
    ...
    public void update(Todo todo){
        ...
        String sql = "UPDATE todo" +
                " SET content = ?" +
                " WHERE num = ?";
        // ? 에 Binding 할 데이터를 Object[] 배열에 순서대로 담아서
        Object[] args = {todo.getContent() , todo.getNum()};
        // execSQL() 메소드의 인자로 Objetc[] 배열을 전달하면 ? 에 순서대로 Binding 한다.
        db.execSQL(sql , args);
        db.close();
    }
    ...
    public Todo getData(int num){
        ...
        // rawQuery() 메소드의 인자로 String[] 배열을 전달하면 ? 에 순서대로 Binding 된다.
        ...
        if(result.moveToNext()){
            ...
            // 0 번쨰 칼럼에 Select 된 문자열을 Todo 객체에 담는다.
            todo.setContent(result.getString(0));

            // 1 번쨰 칼럼에 Select 된 문자열을 Todo 객체에 담는다.
