    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        */
        ...
        binding.addBtn.setOnClickListener(view -> {
            ...
            // 5. Adapter 에 넣어주고 ( 덮어쓰기 )
            adapter.list = newList;

            // 6. 모델의 내용이 바뀌었다고 Adapter 에 알려서 ListView 가 Update 되도록 한다.
            adapter.notifyDataSetChanged();

            // 7. ListView 의 가장 아래 쪽이 화면에 보일 수 있도록 부드럽게 스크롤
        ...
        binding.listView.setOnItemLongClickListener((adapterView, view, index, id) -> {
            new AlertDialog.Builder(this)
                    ...
                    .setPositiveButton("네" , (dialogInterface, i1) -> {
                        ...
                        // 클릭한 Cell 의 ID 를 이용해서 삭제
                        dao.delete((int)id);

                        // 목록을 새로 얻어와서 Adapter 에 넣어주고
                        adapter.list = dao.getList();

                        // 모델의 내용이 바뀌었다고 Adapter 에 알려서 ListView 가 Update 되도록 한다.
                        adapter.notifyDataSetChanged();
                    })
