...
/*
    ...
    Get 방식으로 Http://IP 주소:9000/boot07/android/tweet 에 요청을 하는 예제
    요청 Parameter msg 라는 Parameter 명으로 입력한 문자열이 전송되도록 한다.
    서버가 응답하는 문자열은 오렌지색 EditText 에 출력
*/
...
public class MainActivity extends AppCompatActivity implements Util.RequestListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // activity_main.xml 문서에 정의돤 객체의 참조값 얻어오기
        ...
        sendBtn.setOnClickListener(view -> {
            ...
            // 요청 Parameter 로 전달하기 위해 "msg" 라는 키 값으로 Map 에 담는다.
            Map<String , String> map = new HashMap<>();
            map.put("msg" , msg);

            // Util 을 이용해서 Http 요청을 한다.
            Util.sendGetRequest(0 , "http://192.168.0.32:9000/boot07/android/tweet" , map , this);
        });
        ...
        sendBtn3.setOnClickListener(view -> {
            ...
            // Util 을 이용해서 Http 요청을 한다.
            Util.sendGetRequest(2 , "http://192.168.0.32:9000/boot07/android/tweet3" , map , this);
        });

        Button listBtn = findViewById(R.id.listBtn);
        listBtn.setOnClickListener(view -> {
            Util.sendGetRequest(3 , "http://192.168.0.32:9000/boot07/android/list" , null , this);
        });
    }
    ...
    public void onSuccess(int requestId, Map<String, Object> result) {
        if(requestId == 0){
            ...
            // {"isSuccess":true} 형식의 JSON 문자열은 JSONObject 객체를 이용해서 원하는 데이터를 추출 가능
            try{

                // JSONObject 를 생성해서
                JSONObject obj = new JSONObject(data);

                // "isSuccess" 라는 키 값으로 저장된 true 라는 boolean Type 데이터 얻어내기
                ...
                // data 가 JSON 형식에 어긋나면 예외 발생
                editText.setText(e.getMessage());
            }
        }else if(requestId == 2) {
            String data = (String) result.get("data");

            // data 는 [] 형식의 JSON 문자열 , [] 형식의 JSON 문자열은 JSONArray 객체를 활용
            try {
